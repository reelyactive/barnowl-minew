...
function decodeTransmitterFrame(data, index, raddecs, receiverId) {
  let transmitterId = data.readUIntBE(index, 6).toString(16).padStart(12, '0');
  let transmitterIdLE = data.readUIntLE(index, 6).toString(16).padStart(12,'0');
  ...
  for(let cPacket = 0; cPacket < numberOfDistinctPackets; cPacket++) {
    ...
    let response = data.readUInt8(packetFrameIndex + packetLength + 1);
    let rssi = data.readInt8(packetFrameIndex + packetLength + 2);
    let isRandomAddress = ((response & ADDR_TYPE_MASK) != 0x00);
    ...
    if(packetLength > 0) {
      let packet = (packetLength + 6).toString(16).padStart(4, '0');
      packet += transmitterIdLE;
...
function decode(data, origin, time, options) {
  ...
  let receiverId = data.readUIntBE(6, 6).toString(16).padStart(12, '0');
